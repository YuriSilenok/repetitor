...
@router.callback_query(SettingsStates.select_week and F.data=='set_time_for_edit')
async def set_time_for_edit_handler(callback_query: CallbackQuery, state: FSMContext):
    '''Нажате по кнопке Для изменённых'''
    ...
    await state.set_state(SettingsStates.set_start_time_for_edit)
    await callback_query.message.delete()
    await callback_query.message.answer(
        text=f'Укажите время начала дня недели: {week_day}',
    )

@router.message(SettingsStates.set_start_time_for_edit)
async def set_start_time(message: Message, state: FSMContext):
    ...
    if re.search(constants.TIME_PATTERN, message.text):
        ...
        await state.set_state(SettingsStates.set_end_time_for_edit)
...
@router.message(SettingsStates.set_end_time_for_edit)
