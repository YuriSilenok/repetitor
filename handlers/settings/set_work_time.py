'''Модуль установки времени для всех выранных дней недели'''
...
async def work_time_for_all_handler(callback: CallbackQuery, state: FSMContext):
...
async def set_work_time_for_all(message: Message, state: FSMContext):
...
async def work_time_handler(callback: CallbackQuery, state: FSMContext):
...
async def set_work_time(message: Message, state: FSMContext):
